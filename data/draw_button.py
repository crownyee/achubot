...
class draw_button(Cog_Extension):
    ...
    async def draw_fortune(self, interaction):
        ...
        try:
            await collection.update_one({"_id": user.id},{"$set": {"user_photo": str(user.display_avatar)}})
            ...
            if drawdata['draw_in'] == 1:  
                ...
                fortune_today = random.choice(draw_data.LUCKY_ROLES)
                role_id = int(fortune_roles[draw_data.LUCKY_ROLES.index(fortune_today)])
