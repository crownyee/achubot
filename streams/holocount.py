...
class HoloCog(Cog_Extension):
        ...
        async def holo_SUB():
            ...
            try:
                ...
                while not self.bot.is_closed():
                    self.last_count = float(jdata['SUBCOUNT'])
                    ...
                    if subs_count - self.last_count > 0.0:
                        print(subs_count)
    ...
    @app_commands.command(name='update_sub',description="更新訂閱(admin only)")
        ...
        async def holo_SUB():
            ...
            try:
                ...
                while not self.bot.is_closed():
                    ...
                    if subs_count - self.last_count > 0:
                        print(subs_count)
                        display_count = format(subs_count, ".1f")
                        await self.channel_2.send(f"FUWAMOCO更新訂閱:{display_count}萬")  
                        numbers = str(display_count).split('.')
